...
export const CyberpunkLogin: React.FC<CyberpunkLoginProps> = ({ onSubmit }) => {
    ...
    <div className="fixed inset-0 z-[9999] grid place-items-center bg-black">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          <label className="block text-[11px] uppercase tracking-[0.3em] text-cyan-400">
            Operator ID
            <input
              ref={inputRef}
              value={name}
              ...
              className="mt-2 w-full rounded-lg border border-cyan-500/40 bg-black/60 px-3 py-2 text-sm font-mono text-cyan-100 outline-none transition focus:border-cyan-300 focus:shadow-[0_0_12px_rgba(0,255,255,0.3)]"
            />
          </label>
          ...
          <button
            type="submit"
            className="w-full rounded-lg border border-cyan-500/60 bg-gradient-to-r from-cyan-500/40 to-fuchsia-500/40 px-3 py-2 text-sm font-semibold uppercase tracking-[0.2em] text-cyan-100 shadow-[0_12px_24px_rgba(0,255,255,0.2)] transition hover:shadow-[0_16px_30px_rgba(0,255,255,0.4)] focus:outline-none focus:ring-2 focus:ring-cyan-400/70"
          >
            Enter System
          </button>
        </form>

        <div className="mt-6 border-t border-cyan-500/20 pt-4 text-[10px] uppercase tracking-[0.3em] text-cyan-500/60">
