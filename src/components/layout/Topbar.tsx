  ...
  return (
    <>
      <div className="z-20 flex h-12 items-center justify-between border-b border-cyan-400 bg-black/80 px-4 font-mono text-xs tracking-wide text-cyan-300 shadow-[0_0_6px_#00fff7] backdrop-blur-md md:h-8">
        ...
        <div className="hidden items-center gap-6 text-cyan-200 md:flex">
          <div className="flex items-center gap-1 border-l border-cyan-500 pl-2">
            <svg
              className="h-3.5 w-3.5 animate-pulse text-cyan-400"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
          ...
          <div className="hidden items-center gap-1 border-l border-cyan-500 pl-2 md:flex">
            <svg
              className="animate-flicker h-3.5 w-3.5 text-cyan-400"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
      ...
      <Dialog open={themeDialogOpen} onOpenChange={setThemeDialogOpen}>
        <DialogContent className="border-neon-purple/40 max-h-[85vh] w-[min(92vw,620px)] overflow-y-auto border bg-black/90 p-4 shadow-[0_0_35px_rgba(188,19,254,0.35)] sm:p-6">
          <DialogHeader className="hidden sm:block">
            <DialogTitle className="text-neon-purple text-sm tracking-[0.35em] uppercase">
              Select Environment
            </DialogTitle>
          </DialogHeader>
  ...
  <DropdownMenu>
    <DropdownMenuTrigger asChild>
      <button
        ...
        className="flex items-center gap-2 rounded-lg border border-cyan-500/30 bg-cyan-500/10 px-2 py-1 text-left font-mono text-[11px] tracking-wide text-cyan-200 transition hover:bg-cyan-500/20 focus:ring-2 focus:ring-cyan-400/70 focus:outline-none"
      >
    ...
    <DropdownMenuContent side="bottom" align="end">
      ...
      <DropdownMenuItem disabled className="cursor-default opacity-100">
        ...
        <span className="flex flex-col text-left text-[10px] tracking-[0.3em] text-cyan-400/80 uppercase">
...
const GuestBadge = ({ label }: GuestBadgeProps) => (
  <div className="rounded-lg border border-cyan-500/30 bg-cyan-500/5 px-3 py-1 text-[11px] tracking-[0.3em] text-cyan-400 uppercase">
    {label}
  </div>
);
