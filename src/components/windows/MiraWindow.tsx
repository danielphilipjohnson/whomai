...
const MiraWindow = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    ...
    createMessage(
      'mira',
      'Issue a command like "status", "list apps", or ask me to open something.'
    ),
  ]);
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-gradient-to-br from-gray-950 via-gray-900 to-black text-gray-100">
      <header className="border-neon-purple/40 border-b px-4 py-3">
        <h1 className="text-neon-purple text-lg font-semibold tracking-[0.35em]">MIRA</h1>
        <p className="mt-1 text-xs text-gray-400 uppercase">
          Modular Interface for Reactive Assistance
        </p>
      </header>
          ...
          {messages.map((message) => (
            <div
              ...
              className={`max-w-[85%] rounded-md border px-4 py-3 text-sm leading-relaxed shadow-lg backdrop-blur-sm ${
                message.role === 'mira'
                  ? 'border-neon-purple/50 text-neon-blue self-start bg-black/70'
                  : 'border-neon-green/40 bg-neon-green/10 text-neon-green self-end'
              }`}
            >
              <p className="whitespace-pre-wrap">{message.content}</p>
              <span className="mt-2 block text-[10px] tracking-wide text-gray-500 uppercase">
                {message.role === 'mira' ? 'mira.exe' : 'operator'} •{' '}
                {new Date(message.timestamp).toLocaleTimeString()}
              </span>
            </div>
          ))}
          {isProcessing && (
            <div className="text-neon-purple/70 self-start text-xs tracking-[0.3em] uppercase">
      ...
      <footer className="border-neon-purple/30 shrink-0 border-t bg-black/60 px-4 py-4">
        <div className="mb-3 flex flex-wrap gap-2">
          {QUICK_PROMPTS.map((prompt) => (
            <button
              key={prompt.command}
              className="border-neon-purple/40 text-neon-purple hover:border-neon-blue/60 hover:text-neon-blue rounded-full border px-3 py-1 text-xs tracking-wide uppercase transition disabled:opacity-40"
        ...
        <form onSubmit={handleSubmit} className="flex items-center gap-2">
          <input
            ...
            className="border-neon-purple/40 focus:border-neon-blue flex-1 rounded-md border bg-slate-900/70 px-3 py-2 text-sm text-gray-100 transition outline-none focus:shadow-[0_0_18px_rgba(0,240,255,0.35)]"
            disabled={isProcessing}
            autoFocus
          />
          <button
            type="submit"
            className="border-neon-green/50 bg-neon-green/10 text-neon-green hover:bg-neon-green/20 rounded-md border px-4 py-2 text-sm tracking-wide uppercase transition disabled:opacity-50"
