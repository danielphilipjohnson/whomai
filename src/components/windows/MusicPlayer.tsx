...
export const MusicPlayer = ({ payload }: MusicPlayerProps) => {
		...
		() =>
			bars.map((bar) => (
				<motion.div
					...
					transition={{ repeat: Infinity, repeatType: 'mirror', duration: 1.4, delay: bar * 0.05, ease: 'easeInOut' }}
				/>
			)),
		[]
	);
	...
	return (
		<div
			className="h-full bg-[#05040d] text-emerald-200 font-mono text-xs flex flex-col"
		...
		>
			...
			<div className="flex-1 flex flex-col items-center justify-center gap-6">
				<div className="flex items-end gap-1 h-16">{waveform}</div>
				<div className="flex items-center gap-3">
					<button type="button" className="rounded-full border border-emerald-400/60 px-4 py-1 text-[11px] uppercase tracking-[0.2em] hover:bg-emerald-500/10">Play</button>
					<button type="button" className="rounded-full border border-emerald-400/40 px-4 py-1 text-[11px] uppercase tracking-[0.2em] hover:bg-emerald-500/5">Pause</button>
				</div>
			</div>
		</div>
	);
