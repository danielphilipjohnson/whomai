    ...
    <div className="flex items-center justify-between border-b border-[#15142a] bg-toolbar/90 px-4 py-2">
      ...
      >
        ...
        <button
          ...
          className={clsx(
            buttonBase,
            'text-rose-300 hover:border-rose-500/60 hover:text-rose-100 focus:ring-rose-400/70'
          )}
          aria-label="Delete"
          onClick={onDelete}
      ...
      <motion.div
        ...
        transition={{ duration: 0.2, ease: 'easeOut' }}
      >
        <div className="text-[10px] tracking-wide text-cyan-500/80">{currentPath}</div>
        ...
        <input
          ...
          placeholder="Search"
          className="bg-toolbar-background/70 w-40 rounded-md border border-[#1f1c3b] px-2 py-1 text-[11px] text-cyan-100 placeholder:text-[#3f3e5f] focus:border-emerald-400/70 focus:ring-1 focus:ring-emerald-400/50 focus:outline-none"
        />
      </motion.div>
    </div>
  );
