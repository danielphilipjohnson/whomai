...
export const BootScreen = ({
		...
		if (externalProgress == null) {
			...
			const intervalId = window.setInterval(() => {
				...
				if (currentStep >= steps) {
					...
					setTimeout(() => {
						setLines((prev) =>
							prev.at(-1)?.includes('READY')
								? prev
								: [...prev, stamp('[SYSTEM] READY')]
						);
					}, 140);
				}
			}, interval);
	...
	return (
		<div
			...
			ref={shellRef}
		>
			{!prefersReducedMotion && <div className="pointer-events-none absolute inset-0 boot-scanlines" />}
			...
			>
				<div className="pointer-events-none absolute inset-0 rounded-2xl ring-1 ring-cyan-400/40 shadow-[inset_0_0_38px_rgba(0,255,255,0.22)]" />
				...
				<header className="flex items-center gap-4">
					...
					>
						<div className="absolute inset-0 boot-holo-mask" />
						{Icon ? <Icon className="h-10 w-10 text-cyan-200" /> : <CyberEyeIcon className="h-10 w-10 text-cyan-200" />}
						{!prefersReducedMotion && <div className="boot-glitch absolute inset-0" />}
					</motion.div>

					<div className="flex-1">
						<h2 className="text-lg font-semibold tracking-wide text-cyan-200">CYBERPUNK OS • BOOT SEQUENCE</h2>
					...
					<div aria-live="polite" aria-atomic="true" className="text-right leading-tight">
						<div className="text-2xl font-bold tabular-nums text-cyan-100">{clamped}%</div>
						<div className="text-xs text-cyan-400/60">progress</div>
					</div>
				</header>

				<section className="mt-6 grid grid-cols-3 gap-6" aria-live="polite" aria-label="Boot progress">
				...
				<footer className="mt-4 flex items-center justify-between text-xs text-cyan-400/70">
					...
					<span>
						...
						+{' '}
						<kbd className="rounded border border-cyan-400/30 bg-cyan-500/10 px-1 py-0.5">Space</kbd>{' '}
						for Start Menu
					</span>
				</footer>
			</motion.div>
			...
			{bootChimeSrc && (
				<audio ref={audioRef} src={bootChimeSrc} preload="auto" muted={soundMuted} className="hidden" />
			)}
			...
			<style jsx>{`
				...
				.boot-holo-mask {
					background: linear-gradient(180deg, rgba(34, 211, 238, 0.4), transparent 55%);
					mix-blend-mode: screen;
					animation: boot-shimmer 2.6s ease-in-out infinite;
				}
				...
				.boot-glitch::before {
					...
					animation: boot-glitch 3.4s steps(6) infinite;
				}

				.boot-glitch::after {
					background: linear-gradient(180deg, rgba(217, 70, 239, 0.2), transparent);
					animation: boot-glitch 4.1s steps(5) infinite reverse;
				}

				@keyframes boot-shimmer {
					0% {
						transform: translateY(18%) skewY(2deg);
						opacity: 0.45;
					}
					50% {
						transform: translateY(-18%) skewY(-2deg);
...
function ProgressRing({ value, size = 120, stroke = 8 }: { value: number; size?: number; stroke?: number }) {
	...
	return (
		<svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="text-cyan-900" aria-hidden>
...
function CyberEyeIcon(props: React.SVGProps<SVGSVGElement>) {
	return (
		<svg viewBox="0 0 64 64" aria-hidden="true" {...props}>
			<path d="M8 32c8-12 24-12 32 0-8 12-24 12-32 0Z" fill="none" stroke="currentColor" strokeWidth="2" />
