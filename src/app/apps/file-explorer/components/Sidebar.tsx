...
const containerClasses = 'w-60 bg-[#070614]/80 text-[11px] overflow-y-auto custom-scrollbar shadow-[inset_-2px_0_12px_rgba(0,0,0,0.25)]';
...
export const Sidebar = ({ expandedIds, onToggle, onOpen, onDrop, draggingId }: SidebarProps) => {
	...
	const buildTree = useCallback((folderId: string): TreeNode => {
			...
			<div key={item.id} className="select-none">
				<button
					...
					className={clsx(
						...
						isActive ? 'bg-[rgba(0,255,170,0.12)] text-emerald-300 shadow-[inset_0_0_8px_rgba(0,255,200,0.12)]' : 'hover:bg-[#101026] hover:text-emerald-200',
						isDropTarget ? 'border border-emerald-500/60 bg-[#102020]/60' : 'border border-transparent'
					)}
				...
				>
					...
					/>
					<span className="text-xs" aria-hidden>{iconForFolder(item)}</span>
		...
		<aside className={containerClasses} aria-label="File explorer sidebar">
			<div className="p-2 space-y-1">
				<button
					...
					className={clsx(
						'flex w-full items-center gap-2 px-2 py-1 text-left uppercase tracking-[0.2em] text-[10px] transition',
						rootActive ? 'bg-[rgba(0,255,170,0.12)] text-emerald-300 shadow-[inset_0_0_8px_rgba(0,255,200,0.12)]' : 'hover:bg-[#101026] hover:text-emerald-200',
						isRootDropTarget ? 'border border-emerald-500/60 bg-[#102020]/60' : 'border border-transparent'
					)}
				>
