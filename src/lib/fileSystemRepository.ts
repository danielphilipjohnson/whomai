...
class FileSystemRepository {
  ...
  private generateUniqueName(parentId: string, desiredName: string): string {
    ...
    const siblings = Object.values(this.snapshot.items).filter(
  ...
  private resolveParentId(options: CreateItemOptions): string {
    ...
    const target = Object.values(this.snapshot.items).find(
  ...
  list(path: string) {
    ...
    const directory = Object.values(this.snapshot.items).find(
      (item) => item.path === normalized && item.type === 'folder'
    );
  ...
  createItem(options: CreateItemOptions) {
    ...
    const uniqueName = this.generateUniqueName(
      parentId,
      options.name.trim() || (options.type === 'folder' ? 'New Folder' : 'New File')
    );
    const id = uuidv4();
    ...
    const item: FileSystemItem = {
      ...
      metadata:
        options.type === 'file' ? buildMetadata(uniqueName, options.metadata) : options.metadata,
      createdAt: now,
      updatedAt: now,
      deletedAt: null,
    };
  ...
  restoreItem(id: string, destinationPath?: string) {
    ...
    const preferredPath = destinationPath
      ...
      : item.metadata?.lastParentPath
        ...
        : '/';
    const targetFolder =
      Object.values(this.snapshot.items).find(
        (entry) => entry.path === preferredPath && entry.type === 'folder'
